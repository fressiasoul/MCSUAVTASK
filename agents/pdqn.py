...
class PDQNAgent(Agent):
    ...
    def act(self, state):
        ...
        with torch.no_grad():
            ...
            mask_action = self.env.get_mask_action()
            ...
            if rnd < self.epsilon:
                # TODO
                mask_action[mask_action == 0] = -1e6
                action = self.np_random.choice(self.num_actions, p=softmax(mask_action))
                while action in self.choice_action :
                    action = self.np_random.choice(self.num_actions, p=softmax(mask_action))
                self.choice_action.add(action)
                ...
                # TODO
                Q_a[:, mask_action == 0] = -1e9
                # Q_a = Q_a * (mask_action * -np.inf)
