...
writer = SummaryWriter('./log/{}'.format(datetime.datetime.now().__str__().replace(':', '_'))+" ALL")


def pad_action(act, act_param):
    params = [np.zeros((2,)) for _ in range(500)]
...
@click.command()
@click.option('--seed', default=random.seed(), help='Random seed.', type=int)
...
@click.option('--replay-memory-size', default=5000, help='Replay memory transition capacity.', type=int)
...
@click.option('--tau-actor-param', default=0.1, help='Soft target network update averaging factor.', type=float)
...
def run(seed, episodes, evaluation_episodes, batch_size, gamma, inverting_gradients, initial_memory_threshold,
    ...
    for i in tqdm(range(episodes)):
        ...
        # writer.add_scalar('episode-all/reward', episode_reward, i + 1)
        # writer.add_scalar('episode-all/avg-reward', total_reward / (i + 1), i + 1)
        # writer.add_histogram('episode-all/avg-action-param-1', np.array(action_param)[:, 0], i + 1)
        # writer.add_histogram('episode-all/avg-action-param-2', np.array(action_param)[:, 1], i + 1)
        # writer.add_histogram('episode-all/action', np.array(actions), i + 1)
